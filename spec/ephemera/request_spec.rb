...
RSpec.describe Ephemera::Request do
  describe '#valid?' do
    let(:environment) { {'rack.input' => StringIO.new('')} }
    let(:request)     { Ephemera::Request.new environment }
...
access_token=1234567890123456789012345678901234567890123456789012345678
      ...
      allow(Ephemera::Token).to receive(:find_by_credentials).and_return(nil)
    end

    it 'sources credentials from the access_token parameter' do
      expect(Ephemera::Token).to receive(:find_by_credentials).
    ...
    it 'returns true when a matching token is found' do
      allow(Ephemera::Token).to receive(:find_by_credentials).
